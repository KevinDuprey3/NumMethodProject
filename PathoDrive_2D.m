...
for iteration = 1:5
    ...
    title(['average epidemic ' num2str(iteration)]);
    ...
    xlabel('time (days)','Fontsize',FSize);
    ylabel('Population (fraction of initial)','Fontsize',FSize)
    title(['initially infected X= ' num2str(vine(RandV).X) ' Y= ' num2str(vine(RandV).Y) ' iter ' num2str(iteration)]);
    set(gca,'Fontsize',FSize,'Xlim',[0 61]); 
    box on;grid on
    hold on;
    ...
    fprintf('Cost = %.2f, time to detection = %.2f\n',cost,timeDetect);
